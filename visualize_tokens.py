...
def create_vocabulary_mapping(vocab_size, nanogpt_path=None):
    ...
    # Try to load meta_word.pkl for word tokenizer
    meta_path = "/Users/samueljoseph/Documents/Github/tansaku/nanoGPT/data/knockknock/meta_word.pkl"
    if os.path.exists(meta_path):
        try:
            ...
            if "itos" in meta and "stoi" in meta:
                ...
                # Create mapping from token IDs to actual words
                vocab = {}
...
def analyze_training_data(training_data_path, vocab):
    ...
    # Try to use word tokenizer from meta_word.pkl
    meta_path = "/Users/samueljoseph/Documents/Github/tansaku/nanoGPT/data/knockknock/meta_word.pkl"
    if os.path.exists(meta_path):
        try:
            import pickle
            ...
            if "tokenizer" in meta:
                ...
                print(f"🔢 Unique training tokens: {len(set(training_tokens))}")

                # Map back to our vocabulary indices
                training_vocab_indices = set()
                for token_id in set(training_tokens):
                    if token_id < len(vocab):
                        training_vocab_indices.add(token_id)

                print(
                    f"✅ Found {len(training_vocab_indices)} training tokens in model vocabulary"
                )
...
def create_word_cloud(
    ...
    for idx, score in zip(pos_indices, pos_scores):
        if score > 0:  # Only truly positive values
            ...
            # Convert token to string and clean it
            token_str = str(token)
            if token_str and len(token_str) > 0:
                word_frequencies[token_str] = float(score)
                if training_tokens is None:
                    word_colors[token_str] = (
                        "#000000"  # Black (no training data provided)
                    )
                elif idx in training_tokens:
                    word_colors[token_str] = "#000000"  # Black for training tokens
                else:
                    word_colors[token_str] = "#666666"  # Gray for non-training tokens
    ...
    for idx, score in zip(neg_indices, neg_scores):
        if dim_values[idx] < 0:  # Only truly negative values
            ...
            # Convert token to string and clean it
            token_str = str(token)
            if token_str and len(token_str) > 0 and token_str not in word_frequencies:
                word_frequencies[token_str] = float(
                    score
                )  # Use positive value for frequency
                if training_tokens is None:
                    word_colors[token_str] = (
                        "#CC0000"  # Red (no training data provided)
                    )
                elif idx in training_tokens:
                    word_colors[token_str] = "#CC0000"  # Red for training tokens
                else:
                    word_colors[token_str] = "#FF9999"  # Pink for non-training tokens
    ...
    print(
        f"Dimension {dimension}: {len([c for c in word_colors.values() if c == '#000000'])} positive (black), {len([c for c in word_colors.values() if c == '#CC0000'])} negative (red) tokens"
    )

    if training_tokens is not None:
        # Count training vs non-training tokens
        training_count = len(
            [c for c in word_colors.values() if c in ["#000000", "#CC0000"]]
        )
        non_training_count = len(
            [c for c in word_colors.values() if c in ["#666666", "#FF9999"]]
        )
        print(
            f"  → {training_count} from training data, {non_training_count} NOT from training data"
        )
    ...
    if training_tokens is not None:
        plt.title(
            f"Dimension {dimension} - Black/Red: Training tokens, Gray/Pink: Non-training",
            fontsize=14,
        )
    else:
        plt.title(
            f"Dimension {dimension} - Black: Positive, Red: Negative", fontsize=14
        )
